...
const AlertCollapsible: React.FC<CollapsibleMenuProps> = ({ type, alerts }) => {
    let format = ""; 
      ...
      <AccordionItem value={type} className={format}>
        ...
        <AccordionContent>
          <div className="grid grid-cols-1 gap-4 flex justify-center">
            {alerts.map((alert) => (
              <AlertCard key={alert.location} location={alert.location} description={alert.description} title={alert.title} effective={alert.start_time} endTime={alert.end_time} severity={alert.severity} />
            ))}
          </div>
        </AccordionContent>
      </AccordionItem>
